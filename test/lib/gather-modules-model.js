...
describe('Gather modules model', function() {
  it('Should reject on invalid parameters', function(cb) {
    expectPrms.invalidParameters(gatherModulesModel.gather(), cb);
  });

  it('Should reject on invalid file paths', function(cb) {
    expectPrms.fail(
      gatherModulesModel.gather('', ['fake1', 'fake2']), function(reason) {
  ...
  it('Should match the corresponding file patterns', function(cb) {
    ...
    expectPrms.success(
      gatherModulesModel.gather(config.base, instanceFactory.getNormalPluginPaths()), function(ModulesModel) {
  ...
  it('Should reject on invalid module registration module', function(cb) {
    var config = configurationLoader();
    expectPrms.fail(
      gatherModulesModel.gather(config.base, ['test/samples/invalid-plugins/wrong-arguments-number.js']), function(reason) {
  ...
  it('Should reject on invalid module registration module name', function(cb) {
    var config = configurationLoader();
    expectPrms.fail(
      gatherModulesModel.gather(config.base, ['test/samples/invalid-plugins/wrong-module-name.js']), function(reason) {
        expect(reason + '').to.contain('Invalid arguments');
      },
      cb
    );
  ...
  it('Should match the corresponding file patterns', function(cb) {
    var config = configurationLoader();
    expectPrms.success(
      gatherModulesModel.gather(config.base, ['test/samples/normal-plugin-with-multi-dependencies.js']), function(modulesModel) {
