...
module.exports = klarkApiGenerator;

function klarkApiGenerator(modulesModelInstance) {
  if (!(_.isObject(modulesModelInstance)
    ...
    return Promise.reject('klarkApiGenerator: Invalid parameters');
  }
  ...
  return {
    ...
    getApplicationDependencies: getApplicationDependencies
  };
  ...
  function injectExternalModule(name) {
    ...
    instantiateModules.instantiateExternalDependencies([name]);
  ...
  function injectInternalModuleFromMetadata(moduleName, controller) {
    return injectInternalModule(moduleName, function() {
      var amd = {exports: undefined};
      return moduleDecorator.decorator(amd, moduleName, controller);
    });
  }
