...
function stringifyMissingDependencies(missingDependencies) {
  return _.map(missingDependencies, function(requiredModules, missingDependencyName) {
    return [
      'The module',
      ...
      'does not exist.',
      "You have either misspelled it or did not include it on 'predicateFilePicker'"
    ].join(' ');
  }).join('\n');
}